...
/// <summary>
/// Optional DB-backed pending invite store.
/// Implement this on your cache service when you add the DB schema.
/// Until then, AuditLogService will fall back to in-memory tracking.
/// </summary>
...
public class AuditLogService : IAuditLogService, IDisposable
{
    ...
    // In-memory fallback pending-invite store (used until DB-backed store is implemented)
    ...
    private void PrunePendingInvites()
    {
    ...
    private async Task TrackPendingInviteAsync(AuditLogEntry log)
    {
        ...
        // invite.create and user.invite both represent "pending invite"
        ...
        // DB-backed store (if implemented), otherwise in-memory
    ...
    {
        // Derived log representing inferred invite acceptance.
    ...
    private async Task PollForNewLogsAsync()
    {
        ...
        try
        {
            ...
            if (newLogs.Count > 0)
            {
                ...
                // Track invite sends (pending invites) from any new unique entries
                foreach (var log in newUnique)
                {
                    await TrackPendingInviteAsync(log);
                }

                // Build derived logs (inferred invite accept) and a combined send list
